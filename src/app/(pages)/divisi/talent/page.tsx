  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const talentMembers = data.filter(
          (member: Member) => member.attributes.Divisi === "Talent"
        );
        setMembers(talentMembers);
        setFilteredMembers(talentMembers);
      } catch (err) {
      ...
      <div className="relative h-96 w-full overflow-hidden">
        ...
        <div className="container mx-auto h-full flex flex-col justify-center relative z-10 px-8">
          <div className="text-left max-w-2xl">
            ...
            >
              Kenali anggota tim talent kami yang kreatif dan penuh talenta
            </Typography>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative w-full">
              <Input
                type="text"
                color="blue"
                placeholder="Cari anggota talent..."
      ...
      <section className="px-8 py-12">
        <div className="container mx-auto">
          <Typography variant="h5" color="blue-gray" className="font-medium mb-6">
            {filteredMembers.length} Anggota Talent
          </Typography>
          ...
          {filteredMembers.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Typography className="text-gray-500">
                {searchQuery ? "Coba dengan kata kunci lain" : "Saat ini tidak ada anggota di divisi talent"}
              </Typography>
            </div>
          ) : (
                      ...
                      <div className="mt-auto pt-4 border-t border-gray-100">
                        <div className="flex justify-between items-center">
                          <Button
                            ...
                            onClick={() => window.location.href = `/divisi/talent/${member.attributes.seoURL || member.id}`}
                          >
                            Lihat detail
                          ...
                          <div className="flex items-center gap-2">
                            <FacebookShareButton
                              url={`${window.location.origin}/divisi/talent/${member.attributes.seoURL || member.id}`}
                      ...
                      <div className="col-span-3 w-full">
                        ...
                        <div className="flex flex-wrap items-center gap-4 mt-6">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <FacebookShareButton
                              url={`${window.location.origin}/divisi/talent/${member.attributes.seoURL || member.id}`}
                              quote={`Kenal ${member.attributes.Nama_Anggota} dari Tim Talent`}
                              className="hover:opacity-80 transition-opacity"
                            >
                            ...
                            <TwitterShareButton
                              url={`${window.location.origin}/divisi/talent/${member.attributes.seoURL || member.id}`}
                              title={`Kenal ${member.attributes.Nama_Anggota} dari Tim Talent`}
                              className="hover:opacity-80 transition-opacity"
                            >
                            ...
                            <LinkedinShareButton
                              url={`${window.location.origin}/divisi/talent/${member.attributes.seoURL || member.id}`}
                              title={`${member.attributes.Nama_Anggota} - Tim Talent`}
                              className="hover:opacity-80 transition-opacity"
                            >
